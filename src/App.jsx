...
// ─── ADMIN PANEL ──────────────────────────────────────────────────────────────
function AdminPanel({user,players,attendance,onSignOut,onPlayerAdded}) {
  const TABS=[
  ...
  return <div style={{minHeight:"100vh",background:"#0d1117",color:"#e5e7eb",fontFamily:"'Segoe UI',system-ui,sans-serif",maxWidth:720,margin:"0 auto",paddingBottom:80}}>
    ...
    {/* ── VERWALTUNG TAB ── */}
    {activeTab==="verwaltung"&&<VerwaltungTab players={players} onPlayerAdded={onPlayerAdded} showToast={showToast}/>}
...
// ─── VERWALTUNG TAB ───────────────────────────────────────────────────────────
function VerwaltungTab({players,onPlayerAdded,showToast}) {
...
export default function App() {
  ...
  // ── Echtzeit-Listener für Spieler & Anwesenheit ──
  useEffect(()=>{
    if (!authUser) return;
  ...
  async function handleSignOut() {
    await signOut(auth);
    setPlayers([]); setAttendance({}); setIsAdmin(false);
